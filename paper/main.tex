...
\subsection{Measured Acoustic and Paired Localization Bridges}
A second checksum-verifying adapter processes the TU Darmstadt GFRP acoustic dataset \cite{feldmann2026dataset}: 2,568 unaugmented microphone taps from eight 360 by 280 mm plates, including 2,100 intact and 468 defect-labelled point measurements. Signals are rationally resampled from 51.2 to 16 kHz and converted by the production feature contract. Splitting holds out complete physical plates. Separate healthy/damage robust CORAL maps align synthetic and measured first/second feature moments, while a robust empirical reference uses only gain-insensitive features. For external physical acquisitions, only the upper 10\% measured-reference tail contributes to OOD caution or abstention; simulation sessions are unaffected.

The Bologna acoustic-emission record \cite{zonzini2024dataset} adds 1,000 ray-traced Tx--Rx paths with fourth-order boundary reflections and 27 measured channels from nine known impact positions on a $1\times1\times0.003$ m aluminium plate. Its 2 MHz, 250 kHz domain is not mixed with the low-frequency GFRP waveforms. It instead supports a separate simulation-transfer timing benchmark and a measured few-shot localization calibration. Repository files are accepted only after publisher-declared size and MD5 verification, and manifests preserve record, DOI, license, and selected-file provenance.
...
\subsection{Measured-Domain Bridge Results}
Table~\ref{tab:sim2real} reports two scoped real-data checks. The GFRP detector uses four-fold GroupKFold with complete plates held out; repeated taps, sides, runs, and points from a held-out plate cannot enter training. Balanced accuracy was 0.775, ROC--AUC 0.911, average precision 0.699, F1 0.634, and Brier score 0.097. These are pointwise screening results, not arbitrary multistatic localization.

For the aluminium impact record, direct simulation transfer produced 0.175 m mean localization error. A three-neighbour measured localizer trained on eight positions and tested on the ninth reduced the leave-one-position-out mean to 0.141 m, or 19.6\%. An affine timing correction increased the mean to 0.197 m and was therefore rejected by a negative-transfer guard. The improvement demonstrates the value of a small measured calibration set, but one plate, nine interior positions, and transient impacts cannot validate persistent hidden-defect localization.

\begin{table}[t]
\caption{Scoped Measured-Domain Bridge Results}
\label{tab:sim2real}
\centering
\small
\resizebox{\columnwidth}{!}{%
\begin{tabular}{llll}
\toprule
Record/task & Split & Primary result & Scope limit \\
\midrule
GFRP acoustic detection & held-out plate & Bal. acc. 0.775; AUC 0.911 & pointwise taps \\
Al impact, simulation transfer & held-out position & Mean error 0.175 m & one plate \\
Al impact, measured few-shot & held-out position & Mean error 0.141 m & eight calibration points \\
\bottomrule
\end{tabular}}
\end{table}

\subsection{Software Verification}
The executed verification suite contains 45 passing backend tests. Besides the legacy deterministic simulation, posterior, planning, signal, API, persistence, surrogate, and data-contract tests, it covers dual-control switching, silence evidence, likelihood tempering, constraints, waveform bounds, discrepancy/OOD behavior, measured-reference gating, sim-to-real transport, acoustic dataset parsing, confidence gating, ledger tamper detection, bundle import, truth-sealed NPZ/CSV/WAV replay, additive migration, paired mismatch, injected faults, WebSockets, normalized healthy/damage/unknown state, accumulated sensor unreliability, duplicate-payload rejection, persisted emergency stop, and database hydration of assurance state. Eight frontend utility/configuration tests pass; lint and the production build pass. The only observed backend warning is an upstream HTTP-test-client deprecation notice, not a runtime failure.
