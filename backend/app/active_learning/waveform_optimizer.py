...
class WaveformGeometryOptimizer:
    ...
    def expand(self, candidates: list[Experiment], mode: str = "fast", maximum: int = 72) -> list[Experiment]:
        waveforms = self.RESEARCH_WAVEFORMS if mode == "research" else self.FAST_WAVEFORMS
        result: list[Experiment] = []
        for index, candidate in enumerate(candidates):
            waveform = waveforms[(index + int(candidate.center_frequency_hz // 1_000)) % len(waveforms)]
            ...
            if mode == "research" and len(result) < maximum and index % 3 == 0:
                ...
                result.append(
                    replace(
                        ...
                        waveform=waveforms[(index + 2) % len(waveforms)],
                    )
                )
